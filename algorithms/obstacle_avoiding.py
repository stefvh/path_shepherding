...
def compute_obstacle_avoiding_action(
    i,
    robot_poses,
    herd_poses,
    obstacle_dict,
    ...
    lower_error_margin_distance_nearest = max(2., max_linear_velocity_herd)
    upper_error_margin_distance_nearest = max(2., max_linear_velocity_herd)
    ...
    if np.any(indexes_herd_proximity):
        ...
        if np.any(indexes_robots_proximity):
            ...
            robot_distance_above, robot_distance_under = get_nearest_distance_axis(
                positions=robot_poses[:, :2],
                distances_squared=robots_distances_squared,
                focal_point=nearest_pose[:2],
                heading=heading,
                sign_heading=sign_heading,
                absent_distance=absent_distance,
            )

            # Filter obstacles
            obstacle_nearest_points = []
            ...
            for obstacle_polygon in polygons:
                p = nearest_points(Point(robot_pose), obstacle_polygon)[1]
                obstacle_nearest_points.append(np.array([p.x, p.y]))
            obstacle_nearest_points = np.array(obstacle_nearest_points)
            obstacle_nearest_distances_squared = np.sum(
                (obstacle_nearest_points - robot_pose[:2]) ** 2, axis=1
            )
            obstacle_nearest_proximity_indexes = np.logical_and(
                0.0 < obstacle_nearest_distances_squared,
                obstacle_nearest_distances_squared <= range_detection_robot**2,
            )
            obstacle_nearest_points = obstacle_nearest_points[
                obstacle_nearest_proximity_indexes
            ]
            obstacle_nearest_distances_squared = obstacle_nearest_distances_squared[
                obstacle_nearest_proximity_indexes
            ]

            (
                obstacle_distance_above,
                obstacle_distance_under,
            ) = get_nearest_distance_axis(
                positions=obstacle_nearest_points[:, :2],
                distances_squared=obstacle_nearest_distances_squared,
                focal_point=nearest_pose[:2],
                heading=heading,
                sign_heading=sign_heading,
                absent_distance=absent_distance,
            ...
            distance_above = min(robot_distance_above, obstacle_distance_above)
            distance_under = min(robot_distance_under, obstacle_distance_under)
